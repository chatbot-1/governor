...
import { RedisSlidingWindowLog } from '../../src';
import { createRedis } from './helpers';

describe('RedisSlidingWindowLog (distributed mode) [integration]', () => {
  let redis: Redis;
  ...
  it('allows exactly `limit` requests, then blocks (sequential)', async () => {
    const limiter = new RedisSlidingWindowLog({ redis, limit: 5, windowMs: 60_000 });
  ...
  it('does NOT overcount under heavy concurrency', async () => {
    ...
    const servers = connections.map(
      (conn) => new RedisSlidingWindowLog({ redis: conn, limit: LIMIT, windowMs: 60_000 }),
    );
